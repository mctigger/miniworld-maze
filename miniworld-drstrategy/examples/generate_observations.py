...
def generate_observations(variant, output_dir=None, high_res_full_views=False):
    ...
    for i, action in enumerate(actions):
        ...
        obs_hwc = np.transpose(obs, (1, 2, 0))
        Image.fromarray(obs_hwc).save(f'{output_dir}/gymnasium_step_{i+1}_{action_names[i]}.png')
...
def main():
    parser = argparse.ArgumentParser(description='Generate observations for Nine Rooms environment variants')
    parser.add_argument('variant', choices=['NineRooms', 'SpiralNineRooms', 'TwentyFiveRooms'],
                       help='Environment variant to use')
    parser.add_argument('--output-dir', type=str, default=None,
                       help='Output directory for images (default: {variant}_observations)')
    parser.add_argument('--high-res-full', action='store_true',
                       help='Generate 512x512 high-resolution full environment views')
    
    args = parser.parse_args()
    
    output_dir = generate_observations(args.variant, args.output_dir, args.high_res_full)
