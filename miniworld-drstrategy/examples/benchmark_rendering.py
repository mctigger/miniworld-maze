...
def print_benchmark_results(results, variant):
    ...
    # Table header
    print(f"{'Size':>8} {'Pixels':>10} {'FPS':>8} {'Avg ms':>8} {'Min ms':>8} {'Max ms':>8} {'Std ms':>8} {'Memory':>8}")
    print("-" * 80)
    
    # Table rows
    for size, data in sorted(results.items()):
        print(f"{size:>8} {data['pixels']:>10,} {data['fps']:>8.1f} {data['avg_step_time']*1000:>8.1f} "
    ...
    best_size = next(size for size, data in results.items() if data['fps'] == best_fps['fps'])
    worst_size = next(size for size, data in results.items() if data['fps'] == worst_fps['fps'])
    ...
    if size_32 and size_256:
        ...
        print(f"   📈 Scaling: 256x256 has {pixel_ratio:.1f}x more pixels but only {fps_ratio:.1f}x slower")
...
def benchmark_environment_variants(sizes=None, num_steps=50):
    ...
    print(f"🌟 COMPREHENSIVE ENVIRONMENT BENCHMARK")
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark Nine Rooms rendering performance')
    parser.add_argument('--variant', default='NineRooms', 
    ...
    if args.all_variants:
        all_results = benchmark_environment_variants(sizes=args.sizes, num_steps=args.steps)
    ...
    print(f"\n🎉 Benchmark complete!")
    print(f"✨ Direct rendering approach shows excellent performance scaling!")
