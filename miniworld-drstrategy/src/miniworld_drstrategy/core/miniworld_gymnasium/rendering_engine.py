...
class FrameBufferManager:
    ...
    def get_or_create_framebuffer(self, name: str, width: Optional[int] = None, 
                                 height: Optional[int] = None) -> FrameBuffer:
...
class CameraSystem:
    ...
    def get_config(self, obs_level: ObservationLevel, agent: Any) -> dict:
        ...
        elif obs_level in [ObservationLevel.TOP_DOWN_PARTIAL, ObservationLevel.TOP_DOWN_FULL]:
            return {
                'type': 'orthographic',
...
class RenderingEngine:
    ...
    def render_observation(self, obs_level: ObservationLevel, agent: Any, 
                          entities: list, rooms: list, 
                          frame_buffer: Optional[FrameBuffer] = None) -> np.ndarray:
        ...
        with self.context_manager.rendering_context():
            ...
            self._setup_scene()
            self._render_entities_and_rooms(entities, rooms, agent, 
                                          render_agent=(obs_level != ObservationLevel.FIRST_PERSON))
            img = frame_buffer.resolve()
        
        return img
    
    def render_top_view(self, agent: Any, entities: list, rooms: list, 
                       POMDP: bool = False, render_agent: bool = True,
                       frame_buffer: Optional[FrameBuffer] = None) -> np.ndarray:
        """Render top-down view of the environment."""
        if frame_buffer is None:
            frame_buffer = self.framebuffer_manager.get_or_create_framebuffer('topview')
        
        obs_level = ObservationLevel.TOP_DOWN_PARTIAL if POMDP else ObservationLevel.TOP_DOWN_FULL
        camera_config = self.camera_system.get_config(obs_level, agent)
    ...
    def _render_entities_and_rooms(self, entities: list, rooms: list, 
                                  agent: Any, render_agent: bool = True):
