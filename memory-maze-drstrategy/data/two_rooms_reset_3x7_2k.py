...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/common/home/fd213/datasets/memory-maze-3x7-reset-2k')
    ...
    env = tasks.memory_maze_two_rooms_3x7_fixed_layout(global_observables=True, top_camera=args.top_camera, **kwargs)
    num_actions = 6
    ...
    while episode_idx < args.episode_start + args.num_episodes:
        ...
        while True:
            ...
            agent_pos = timestep.observation['agent_pos']
            x, y = agent_pos[0], agent_pos[1]
            if (0.5 <= x <= 2.5 or 4.5 <= x <= 6.5) and (0.5 <= y <= 2.5):
                break
        ...
        try:
            ...
            switch_room_step_1 = np.random.randint(250, 301)
            switch_room_step_2 = np.random.randint(1650, 1701)
