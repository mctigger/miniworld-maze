...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/home/jypark/datasets/memory-maze-7x19-reset-copy-new')
    ...
    env = tasks.memory_maze_ten_rooms_7x19_fixed_layout_random_goals(global_observables=True, top_camera=args.top_camera, **kwargs)
    num_actions = 6
    ...
    while episode_idx < args.episode_start + args.num_episodes:
        ...
        switch_room_steps = [np.random.randint(i * 60 + 50, i * 60 + 61) for i in range(9)]
        switch_room_steps.append(599)
        copy_step = np.random.randint(50, 200)

        try:
            for switch_room_step in switch_room_steps[:-1]:
                ...
                timestep, local_obs_list, local_action_list, local_step_counter = switch_room(env, timestep, switch_room_direction, i_max=5)
                obs_list.extend(local_obs_list)
                action_list.extend(local_action_list)
                step_counter += local_step_counter

            switch_room_step = switch_room_steps[-1]
            timestep, local_obs_list, local_action_list, local_step_counter = explore_room(env, timestep, num_steps=switch_room_step - step_counter)
            obs_list.extend(local_obs_list)
            action_list.extend(local_action_list)
            step_counter += local_step_counter
