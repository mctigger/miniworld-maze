...
def get_map_coord(timestep, maze_size=7):
...
def turn_to_target(env, timestep, target_pos, max_steps):
    ...
    while np.inner(agent_dir, target_dir) < np.cos(np.pi / 18):
        ...
        agent_dir = timestep.observation['agent_dir']
        map_coord = get_map_coord(timestep)
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/common/home/fd213/datasets/memory-maze-7x7-coord-reset-copy')
    ...
    while episode_idx < args.episode_start + args.num_episodes:
        ...
        try:
            ...
            copy_steps = np.random.randint(1, 601, size=num_steps)
            for copy_step in copy_steps:
                obs_list.append(obs_list[copy_step])
                action_list.append(action_list[copy_step - 1])
